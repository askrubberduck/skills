...
v2.5.0. Every push runs `scripts/validate-distribution.py --self-test`: manifests parse, every
skill is linked where cloud sessions look, cross-references resolve, generated files are current,
and each check still rejects the corruption it exists for. The script checks structure only.
