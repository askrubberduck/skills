<!-- translation-source: README.md sha256=d63c858a581c0af26b558dccbdc90afb2faa469eea84b5b1fafa5890453f66ea -->
...
всё время спрашивает, где твой вывод может оказаться неверным. В том числе — нужно ли это было делать
вообще.
...
разрешение не нужно. Исключение — собственный конфиг утки: если `[learn].discover` оставлен в `auto`,
`duck-learn` сам ставит новую модель на испытание и вносит вердикты испытаний в
...
4. **Доказательство**, `duck-proof`. Исполняемый код сначала проходит пинг-понг `duck-race` с моделью
   другого семейства. Атакуй цель, поведение и итоговое устройство. Где нужно,
   `duck-break` пытается всё сломать. Что-то поправил — перезапусти затронутые
...
`duck-campaign` берёт большую цель, бэклог или список того, чего не хватает, и ведёт сразу много задач. Каждая
доходит до релиза, не дожидаясь остальных.
...
пропустили оба ревьюера? `scripts/ledger.py remaining <gate_id>` оценивает это по их общим
...
`duck-learn`. Во что обойдётся этот гейт? `cost <setup>` смотрит на прошлые. Вышла новая модель? `roster <models|->` покажет модели
хоста, которых нет ни в одном списке ролей, ни на испытании, ни в реестре; после теневых гейтов `promote` скажет, заменит ли она ревьюера своего
