...
ревьюера? `skills/duck-review/scripts/ledger.py remaining <gate_id>` оценивает это по их общим
находкам. Какое семейство что лучше ловит? `precision`. Кто ревьюит следующим? `pick <stage>`
выбирает по записанным находкам в минуту — среди тех, кого требует гейт. Помогло ли изменение?
...
список или уйдёт. `skills/duck-review/scripts/ledger.py --self-check` прогоняет каждую команду на
тестовых данных и проверяет вывод. Только стандартная библиотека. `-` значит «неизвестно», а
