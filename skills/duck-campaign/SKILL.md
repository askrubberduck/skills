...
4. **Plan each packet** via `duck-plan` before any build starts. **No co-authorship line, no
...
5. **Launch builds in parallel worktrees** (`.worktrees/<task>/` — never the shared checkout), cheap
   executor agents for mechanical slices, one session per packet. Apply `duck-diet`
...
6. **Hand off into a running loop, never into silence.** State the campaign roster (packet, worktree,
   ...
   packets have merged, `duck-sweep` clears the worktrees they leave behind.
